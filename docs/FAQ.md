...
n_individuals = 8124
k = 5
stub_coords = pd.DataFrame(
    np.random.randn(n_individuals, k),
    columns=[f'Dim{i+1}' for i in range(k)]
)
stub_coords.to_csv('data/processed/mca_coords.csv', index=False)
```
...
# B récupère et merge
git checkout dev-B
git pull origin main  # récupère travail de A
# ... continue travail B ...
git checkout main
git merge dev-B
git push origin main
...
**Alternative simple** : Travaillez tous les deux sur `main` si pas de Git compliqué.
...
**Durée rédaction** : J5-J6 (après code terminé)
...
- k = 5 axes (inertie cumulée 92.4%)

Top contributions Dim 1 : odor, spore-print-color
Top contributions Dim 2 : gill-color, cap-shape

Les coords sont dans le repo, tu peux lancer 04 et 05 !
```

### Message B → A (fin J4)
```
✅ Clustering + Discriminante OK !

Résultats :
- 3 clusters identifiés (profils distincts)
- LDA : accuracy 98.7% (CV 5-fold)

Check reports/figures/cluster_* et da_*
Prêt pour rédaction sections 5-6.
```

---

**Dernière mise à jour** : J0 (création projet)
